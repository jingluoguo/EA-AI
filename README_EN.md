...
> Can a model extract entities, events, state, rules, and questions from text, then reason over those structures to answer?
...
Core intermediate structures:

- `ENTITY`: objects such as people, items, containers, and places.
- `FRAME/ROLE`: historical events and event roles in source order.
- `STATE`: current world state projected from events, with later state overwriting earlier state.
- `QUERY`: a computable representation of the user question.
- `INTENT`: a mental-state hypothesis inferred from observed behavior.
- `RULE`: the rule inferred during reasoning.

`REL` and `EVENT` may still appear in CLI output, but they are compatibility/readability views. New reasoning should be built on `FRAME/ROLE/STATE/QUERY`.
...
raw text
  ...
  -> world/state.py projects FRAME into current STATE
