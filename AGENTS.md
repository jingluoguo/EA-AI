...
- 训练优先：遇到新失败样例时，先补训练/反馈数据，记录 `observation`、上下文、已知世界状态、信念状态、时间顺序、来源、条件、因果、期望 `INTENT/QUERY/STATE/FRAME/ROLE` 或答案，再用数据集评估现有能力；只有数据证明结构标签缺失或模型槽位不足时，才改 parser、state、query 或 inference。
...
- 构建理想模型：优先把语言归一到实体/类型/属性、`FRAME/ROLE`、`STATE`、时间顺序、极性与修正、来源与信念、条件与因果、数量与比较、指代与焦点、`QUERY`、`INTENT`、置信度和规则等中间结构。`EVENT/REL` 只能作为兼容线性化视图，不能成为新增推理能力的主模型。例如“谁把芯片放进托盘？”、“芯片是谁放进托盘的？”、“芯片被谁放进托盘的？”应归一为同一个事件角色查询。
